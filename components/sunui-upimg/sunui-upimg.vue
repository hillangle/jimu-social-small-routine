	...
	<view class="sunui-uploader-bd">
		<view class="sunui-uploader-files">
			...
			<view v-show="upload_len < upload_count" hover-class="sunui-uploader-hover" class="sunui-uploader-inputbox" @click="chooseImage" :style="upload_img_wh">
				<view><text class="iconfont icon-mn_shangchuantupian">+</text></view>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	props: {
		...
		url: {
			type: String,
			default: attrUtl + '/filesystem/uploadFile'
		},
		// 上传样式宽高
		upload_img_wh: {
			type: String,
	...
	const uploadTask = uni.uploadFile({
		...
		success: function(res) {
			...
			if (res.statusCode == 200 && data.success) {
				if (success) {
					var path  = attrUtl+'/filesystem/downloadFile?attaOid='+data.resultData.oid;
					//console.log('uploadsuccess' + path)
					success(path);
					//success( data.resultData.oid  );
				}
			} else {
				if (fail) {
					fail(data);
